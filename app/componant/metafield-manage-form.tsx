import { Database, Search, ArrowLeft, Download, Trash2, CheckCircle2, FileText, AlertTriangle } from "lucide-react";

export const MetafieldFetcherUI = ({
  objectType, setObjectType, queryMap, fetchMetafields, loading, isDeleting, metafields, resetToHome, hasSearched,
}) => (
  <div className="flex flex-col mb-6 bg-white p-5 rounded-lg border border-[#dfe3e8] shadow-sm">
      ...
      {metafields.length === 0 && (
        <div className="flex-1 w-full">
          ...
          <select
            ...
            className="w-full h-11 border border-[#babfc3] px-3 py-2 rounded-md focus:border-black focus:ring-1 focus:ring-black outline-none transition-all disabled:bg-[#fafbfb] text-base"
          >
            {Object.entries(queryMap).map(([key]) => (
              <option key={key} value={key}>{key.charAt(0).toUpperCase() + key.slice(1)}s</option>
            ))}
          </select>
        </div>
      )}

      <div className={`flex-shrink-0 w-full sm:w-auto ${metafields.length > 0 ? "flex-1" : ""}`}>
        {metafields.length === 0 ? (
          <button
            disabled={loading || isDeleting}
            onClick={fetchMetafields}
            className="w-full sm:w-auto h-11 bg-black text-white px-6 py-2 rounded-md font-bold text-base hover:bg-gray-800 disabled:bg-gray-200 transition-all flex items-center justify-center gap-2"
          >
          ...
          <button
            onClick={resetToHome}
            className="h-11 px-4 py-2 text-base font-bold border border-[#dfe3e8] rounded-md hover:bg-gray-50 inline-flex items-center justify-center gap-2"
          >
    ...
    {hasSearched && metafields.length === 0 && !loading && (
      <div className="mt-4 p-4 bg-[#fff4e5] border border-[#ffebcc] text-[#664d03] rounded-md flex items-center gap-3 text-sm animate-in fade-in">
        ...
        <span><strong>No metafields found.</strong> Try selecting a different object type.</span>
      </div>
    )}
  </div>
);
    ...
    <div className="p-5 space-y-4">
            ...
            {removeMode === mode.id && mode.id !== 'all' && (
              <div className="mt-3 pt-3 border-t border-gray-200 space-y-3 animate-in fade-in duration-200">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={handleDownloadTemplate} className="text-xs font-bold text-gray-500 uppercase flex items-center gap-1 hover:text-black transition-colors">
                ...
                <div className="relative group">
                  <input
                    ...
                    onChange={mode.id === 'update' ? handleupdateCSVUpload : handleCSVUpload}
                    disabled={isDeleting}
                ...
                {csvData > 0 && (
                  <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-100 rounded-md text-[#008060] text-sm font-bold">
                    ...
                    {csvData} Records Loaded Successfully
                  </div>
                )}
              </div>
            )}
      ...
      <div className="flex items-center gap-3 pt-5 border-t border-[#f1f2f3]">
        <button
          ...
          className="flex-1 bg-red-600 text-white h-11 rounded-md font-bold text-base hover:bg-red-700 disabled:bg-gray-200 disabled:text-gray-400 transition-all flex items-center justify-center gap-2"
        >
          {isDeleting ? "Processing..." : <><Trash2 size={18} /> Run Operation</>}
        </button>
        {!isDeleting && (
          <button className="px-6 h-11 text-base font-bold border border-[#dfe3e8] rounded-md hover:bg-gray-50 transition-all" onClick={backToSelectedFeild}>
            Back
          </button>
        )}
      </div>
...
export const CompletionResultsUI = ({ results, downloadResultsCSV, resetToHome, removeMode }) => (
  <div className="mt-8 p-6 bg-[#f1f8f5] border border-[#bbe5b3] rounded-xl flex flex-col items-center text-center animate-in zoom-in-95">
    ...
    <div className="flex items-center gap-3">
      {results.length > 0 && (
        <button
          className="bg-black text-white px-6 py-2 rounded-md font-bold text-sm hover:bg-gray-800 transition-all flex items-center gap-2 shadow-md"
      ...
      <button
        className="bg-white text-gray-900 border border-[#dfe3e8] px-6 py-2 rounded-md font-bold text-sm hover:bg-gray-50 transition-all"
        onClick={resetToHome}
      >
        Done
      </button>
