...
export async function removeSpecificMetafield(
  ...
  //   // ---------- Filter values ----------
  //   const filteredList = existingList.filter((v) => !removeList.includes(v));
  ...
  //   console.log(removeList, ".........removelsit");
  //   return {
  //     id: ownerId,
  ...
  if (flag1) {
    ...
    /* =====================================================
       RESOLVE REMOVE VALUES (HANDLE → ID)
    ===================================================== */
    ...
    console.log("🗑️ REMOVE IDS:", resolvedRemoveIds);
    console.log("📦 EXISTING IDS:", existingList);
  ...
  if (!flag) {
    ...
    if (!resolved) {
      return {
        ...
        value,
        success: false,
        errors: `Could not find ${objectType}: ${id}`,
      };
    }
    ownerId = resolved;
  }
