...
export default function TagManager() {
  ...
  const [removalMode, setRemovalMode] = useState<"global" | "specific">("global");
  ...
  // -- Helpers --
  function filterTagsBasedOnConditions(allTags: string[], conditions: any[], matchType: string) {
    ...
    const match = (tag: string, cond: any) => {
      ...
      switch (matchType) {
        case "exact": return t === value;
        case "start": return t.startsWith(value);
        case "end": return t.endsWith(value);
        default: return t.includes(value);
      }
    };
  ...
  useEffect(() => {
    ...
    if (data.mode === "remove-specific") {
      const updatedResults = data.results.map((item: any) => ({ ...item, row: currentrow }));
    ...
    if (data.mode === "remove-global") {
      ...
      if (data.hasNextPage) {
        ...
        fd.append("cursor", data.nextCursor);
        setTimeout(() => { fetcher.submit(fd, { method: "POST" }); }, 200);
      } else {
        setIsRemoving(false);
      }
      return;
  ...
  useEffect(() => {
    let results: any[] = [];
    if (globalResult.complete && globalResult.results.length > 0) results = globalResult.results;
    ...
    // Use pure fetch to avoid 404/navigation issues
    fetch("/api/add/db", {
      method: "POST",
  ...
  const handleDropZoneDrop = useCallback((_dropFiles: File[], acceptedFiles: File[], _rejectedFiles: File[]) => {
    ...
    Papa.parse(file, {
      ...
      complete: (res) => {
        ...
        const values = res.data.map((row: any) => {
        ...
        if (hasInvalidGid) { handleClearCSV(); return; }
        ...
        if (values.length === 0) {
          setAlert({ active: true, title: "Valid Record Not Found", message: "No valid records found in the CSV file. Please follow the CSV Format", tone: 'critical' });
          handleClearCSV();
          return;
        }
  ...
  return (
    ...
    >
      <Layout>
              ...
              <BlockStack gap="300">
                  ...
                  <InlineStack key={i} gap="200" align="center">
                    <div style={{ flex: 1 }}>
                      <TextField
                        ...
                        connectedRight={
                          conditions.length > 1 && (
                            <Button icon={XIcon} onClick={() => removeCondition(i)} disabled={isActionDisabled || fetchedItems.length > 0} />
                          )
                        }
                      />
                    </div>
                ...
                <ButtonGroup fullWidth>
                  ...
                  {fetchedItems.length > 0 && (
                    <Button onClick={resetAll} disabled={isActionDisabled} >Reset</Button>
                  )}
                </ButtonGroup>
              </BlockStack>
            </LegacyCard>
        ...
        <Layout.Section>
          <BlockStack gap="500">
            ...
            {isFetching && (
              <LegacyCard sectioned>
                <BlockStack align="center" inlineAlign="center" gap="400">
                  <Spinner size="large" />
                  <Text as="h3" variant="headingMd">Scanning Store Tags</Text>
            ...
            {/* 2. Empty State */}
            {!isFetching && !noTagsFound && fetchedItems.length === 0 && !isRemoving && !globalResult.complete && !specificEnd && (
              <LegacyCard sectioned>
                <EmptyState
                  heading="Ready to Search"
                  image="https://cdn.shopify.com/s/files/1/0262/4071/2726/files/emptystate-files.png"
                >
                  <p>Select an object type on the left, enter tags, and click Fetch Tags.</p>
                </EmptyState>
              </LegacyCard>
            )}
            ...
            {!isFetching && fetchedItems.length > 0 && !isRemoving && !globalResult.complete && !specificEnd && (
              <BlockStack gap="400">
                <LegacyCard title={`Select Tags to Remove (${fetchedItems.length} found)`} sectioned>
                  <BlockStack gap="200">
                    ...
                    {selectedTags.length > 0 && (
                      <Button variant="plain" tone="critical" onClick={() => setSelectedTags([])}>Clear Selection</Button>
                    )}
                  </BlockStack>
                </LegacyCard>
                  ...
                  <LegacyCard title="Removal Method" sectioned>
                    <BlockStack gap="400">
                      <ChoiceList
                        title="Choose method"
                        choices={[
                          { label: 'Global Removal (All items store-wide)', value: 'global' },
                          { label: 'Specific Removal (From CSV)', value: 'specific' },
                        ]}
                      ...
                      {removalMode === 'specific' && (
                        <Box padding="400" background="bg-surface-secondary" borderRadius="200">
                          <BlockStack gap="300">
                            ...
                            <DropZone onDrop={handleDropZoneDrop} accept=".csv" allowMultiple={false} disabled={isActionDisabled}>
                              {fileName ? (
                                <DropZone.FileUpload actionTitle="Replace file" />
                              ) : <DropZone.FileUpload actionTitle="Add file" />}
                            </DropZone>
                            {fileName && <Text as="p" tone="success">{fileName} — {csvIds.length} records.</Text>}
                          </BlockStack>
                        </Box>
                      )}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (!csvIds.length && removalMode !== 'global') {
                            setAlert({ active: true, title: "Missing CSV", message: "Upload CSV first", tone: "critical" });
                            return;
                          }
                          setModalOpen(true);
                        }}
            ...
            {isRemoving && (
              <LegacyCard sectioned>
                <BlockStack align="center" inlineAlign="center" gap="500">
                  <Spinner size="large" />
                  <Text as="h3" variant="headingMd">Removing...</Text>

                  {/* Percentage Text Block */}
                  {(() => {
                    const percentage = removalMode === 'global' ? 50 : Math.round((finalSpecificResults.length / csvIds.length) * 100);
                    return (
                      <BlockStack gap="200" align="center" inlineAlign="center">
                        {removalMode !== 'global' ? <div style={{ width: '100%', minWidth: '300px' }}>
                          <ProgressBar progress={percentage} tone="highlight" />
                        </div> : null}
                        {removalMode !== 'global' ? <Text as="span" variant="bodyMd" fontWeight="bold">{percentage}%</Text> : null}
                      </BlockStack>
                    );
                  })()}

                  <Text as="p" tone="subdued">
                    {removalMode === 'global' ? `${globalResult.totalProcessed} items processed` : `${finalSpecificResults.length} / ${csvIds.length} items processed`}
                  </Text>
                </BlockStack>
              </LegacyCard>
            )}
            ...
            {(globalResult.complete || specificEnd) && !isRemoving && (
              <LegacyCard sectioned>
                <BlockStack align="center" inlineAlign="center" gap="500">
                  <Badge tone={globalResult.success ? 'success' : 'critical'}>Operation Complete</Badge>
                  ...
                  <InlineStack gap="300">
                    <Button onClick={downloadResultCSV} variant="primary">Download Results</Button>
                  ...
                  >
                    {[...finalSpecificResults].reverse().map((r, i) => (
                      <IndexTable.Row key={i} id={i.toString()} position={i}>
                        <IndexTable.Cell>{finalSpecificResults.length - i}</IndexTable.Cell>
                        <IndexTable.Cell>{r.row}</IndexTable.Cell>
                        <IndexTable.Cell><Badge tone={r.success ? 'success' : 'critical'}>{r.success ? 'Success' : 'Failed'}</Badge></IndexTable.Cell>
      ...
      >
        <Modal.Section>
          <Text as="p">Are you sure you want to remove {selectedTags.length} tag(s)? This action cannot be undone immediately.</Text>
        </Modal.Section>
      </Modal>
    </Page>
  );
