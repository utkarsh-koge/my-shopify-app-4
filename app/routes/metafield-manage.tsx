...
import {
  ...
  MetafieldLoadingUI,
} from "app/component/MetafieldManageForm";
...
export default function SingleMetafieldViewer() {
  ...
  const handleupdateCSVUpload = async (e) => {
    ...
    const rows = dataRows
      .map((cols) => {
        ...
        console.log(gidObjectType, type, '..................gidObjectType')
        if (
          gidObjectType &&
          gidObjectType !== type
        ) {
  ...
  const handleDownloadTemplate = () => {
    ...
    if (
      ...
      // Single-cell list values (comma-separated string)
      ...
      const rows = [
        `${header},value`,
        ...sampleValues.map(
          (val, i) => `${val},"${rightColumnSamples[i]}"`
        ),
      ];
    ...
    if (removeMode === "update" && !selectedMetafield?.type?.name?.startsWith('list.')) {
      const rightColumnSamples = [
        "value-1",
        "value-2",
        "value-3",
  ...
  useEffect(() => {
    ...
    if (
      progress === 100 &&
      !isDeleting &&
      (removeMode === "all" || removeMode === "specific" && listRemoveMode !== 'partial')
    ) {
      const TrueResult = results.filter((r) => r?.success);

      // Flatten the structure for restore
      const formattedResults = TrueResult.map((r) => {
        // If data is nested in r.data (from removeSpecificMetafield)
        ...
        // If data is already flat or in a different format
        return r;
      });
