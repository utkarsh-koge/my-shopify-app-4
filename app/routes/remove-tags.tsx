...
import Navbar from "app/component/AppNav";
    ...
    <AppProvider embedded apiKey={apiKey}>
      <div className="min-h-screen bg-[#f1f2f4] p-6 font-sans relative">
        <div className="max-w-6xl mx-auto">
          ...
          <div
            className={`grid grid-cols-1 lg:grid-cols-3 gap-6 ${isRemoving ? "opacity-50 pointer-events-none" : ""}`}
          >
                  ...
                  {conditions.map((c, i) => {
                    ...
                    return (
                      <div key={i} className="relative">
                        <input
                          disabled={isActionDisabled || fetchedItems.length > 0}
                          className={`w-full border px-3 py-2 rounded-lg text-gray-900 focus:ring-2 focus:ring-gray-800 outline-none transition-all disabled:bg-gray-100 ${
                            isDuplicate
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                !specificEnd && (
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-10 text-center flex flex-col items-center justify-center h-full min-h-[400px]">
                    ...
                    <p className="text-gray-500 max-w-sm mx-auto">
                      Select an object type on the left, enter the tags you want
                      to find, and click "Fetch Tags" to get started.
                    </p>
                  </div>
                )}
              ...
              {noTagsFound && !isFetching && fetchedItems.length === 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-8 text-center text-blue-800">
                  ...
                  <p className="text-blue-600 mt-1">
                    Try adjusting your search conditions or Match Type.
                  </p>
                  <button
                    onClick={handleCancel}
                    ...
                    {selectedTags.length > 0 && (
                      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 animate-in fade-in slide-in-from-top-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                          <label
                            className={`relative flex items-start p-4 cursor-pointer rounded-lg border transition-all ${removalMode === "global" ? "border-black bg-gray-50 ring-1 ring-black" : "border-gray-200 hover:border-gray-300"}`}
                          >
                            <input
                              type="radio"
                            ...
                            />
                            <div>
                              <span className="block font-medium text-gray-900 cursor-pointer">
                                Global Removal
                              </span>
                              <span className="block text-xs text-gray-500 mt-1">
                                Remove tags from ALL items store-wide.
                              </span>
                            </div>
                          </label>

                          <label
                            className={`relative flex items-start p-4 cursor-pointer rounded-lg border transition-all ${removalMode === "specific" ? "border-black bg-gray-50 ring-1 ring-black" : "border-gray-200 hover:border-gray-300"}`}
                          >
                            <input
                              type="radio"
                              className="mt-1 mr-3 accent-black"
                            ...
                            <div>
                              ...
                              <span className="block text-xs text-gray-500 mt-1">
                                Only remove tags from a specific CSV list.
                              </span>
                            </div>
                          </label>
                        </div>
                          ...
                          <div className="bg-gray-50 rounded-lg p-5 border border-gray-200 mb-6">
                            {csvIds.length === 0 ? (
                              <>
                                <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                                  ...
                                  <button
                                    ...
                                    className="ml-auto cursor-pointer text-xs text-[#008060] underline flex items-center gap-1 hover:text-black"
                                  >
                              ...
                              <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-100 rounded-md text-[#008060] text-sm font-bold">
                                ...
                                <span>
                                  {csvIds.length} Records Loaded Successfully
                                </span>
                                <button
              ...
              {isRemoving && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-10 text-center flex flex-col items-center justify-center">
                  ...
                  {removalMode === "specific" && (
                    <p className="text-sm text-gray-500 mt-2 font-medium">
                      {finalSpecificResults.length} / {csvIds.length} items
                      processed...
                    </p>
                  )}

                  <p className="text-xs text-gray-400 mt-4">
                    Please keep this window open while we update your store.
                  </p>
                </div>
              )}
              ...
              {(globalResult.complete || specificEnd) && !isRemoving && (
                <div className="space-y-6 animate-in zoom-in-95 duration-300">
                  <div
                    className={`rounded-xl p-10 text-center border ${globalResult.success ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
                  >
                    <CheckCircle
                      size={48}
                      className={`mx-auto mb-4 ${globalResult.success ? "text-green-600" : "text-red-600"}`}
                    />
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                      <button
                        ...
                        className="bg-black text-white px-8 py-3 rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg"
                      >
                        <FileText size={20} /> Download Results CSV
                      </button>
                      <button
                        onClick={handleCancel}
                        className="bg-white text-gray-900 border border-gray-300 px-8 py-3 rounded-lg font-bold hover:bg-gray-50"
                      >
                        Start New Task
                      </button>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-in slide-in-from-bottom-4">
                  ...
                  <div className="max-h-80 overflow-y-auto p-4 space-y-2">
                    {[...finalSpecificResults].reverse().map((r, idx) => (
                      <div
                        key={idx}
                        className={`text-xs p-3 rounded-md flex flex-col gap-1 border ${r.success ? "bg-white border-gray-100" : "bg-red-50 border-red-100"}`}
                      >
          ...
          <AlertModal modalState={alertState} setModalState={setAlertState} />
        </div>
      </div>
    </AppProvider>
  );
